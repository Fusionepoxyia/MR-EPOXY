...
• SKU: UNOCON100 — EPOXY UNO USO CONSTRUCCION
...
TODA tu respuesta —incluidos saludos, recomendaciones y mensajes de cortesía— debe estar en ${langName}.`;
    ...
    if (data.candidates && data.candidates[0]?.finishReason === "SAFETY") {
      return Response.json({
        content: [{ type: "text", text: "Disculpa, no puedo responder esa pregunta. ¿Hay algo sobre nuestros productos epóxicos en lo que pueda ayudarte?" }]
      });
    }
    ...
    return Response.json({
      content: [{ type: "text", text: "Disculpa, tuve un problema al procesar tu pregunta. ¿Podrías intentarlo de nuevo?" }]
    });
